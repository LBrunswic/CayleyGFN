...
class ReplayBuffer(tf.keras.callbacks.Callback):
    ...
    def memorize(self):
        if self.memory['paths_true'] is None:
            ...
            self.memory['paths_true'] = tf.concat([self.memory['paths_true'], self.model.paths_true[0,...,0].numpy()], axis=0)
            self.memory['paths_embedded'] = tf.concat([self.memory['paths_embedded'], self.model.paths[0,...,0].numpy()], axis=0)
            self.memory['paths_reward'] = tf.concat([self.memory['paths_reward'], self.model.paths_reward[...,0].numpy()], axis=0)
            self.memory['path_init_flow'] = tf.concat([self.memory['path_init_flow'], self.model.path_init_flow[...,0].numpy()], axis=0)
